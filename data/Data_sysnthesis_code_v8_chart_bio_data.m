...
load('biological_format_data.mat')
...
% + 4 (bio data) - 122-126: Lipid #1, 127-131: Lipid #2
%                 , 132-136: Musle #1, 137-141: Musle #2 : color 24-27
Normalized_colors=(Ref_data)./(white_bg);
for i=122:5:141
    Normalized_colors(:,i:i+4) = repmat(bio_sample_data(:, (i-122)/5+1), 1, 5);
end
...
%         save(strcat('result_data/','color_data.mat'),'w_length','N_colors', 'filtered_w_length', 'Filtered_colors','-v7.3')
%         return

        %% 색상 위치 change, list에 포함된 번호 color를 맨 뒤로 보내기(23 color + bio = 24 기준)
        color_loc = [ 1 5 15 24 ];
        F_front = [];
        F_back = [];
        N_front = [];
