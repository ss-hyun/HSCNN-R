...
input_channel=30;
output_channel=100;
% base_channel=length(wavelength);
...
for p=1:1:size(prefix,1)
    ...
    if strcmp(pre, 'train') == 1
        numbers_of_data = 5;
        base_size_list = [ 500 500 800 800 ];
        seg_size_list = [ 50 25 40 80 ];
        start_colors = 1;
        end_colors = 15;
    else
        numbers_of_data = 2;
...
%        filter_band_width=round(linspace(3,8,base_channel));
...
%         figure(35), imagesc(seg_base_img),axis image, colormap('bone')
        ...
        % end
        %
        % figure(36), imagesc(random_color_img,[0 end_colors]),axis image, colormap('bone')
...
%         filter_pos=round(linspace(4,base_channel-6,input_channel));
...
%                 temp_filtered_color(tt,1)=sum(sum(temp_color.*filters(:,tt)))/sum(sum(squeeze(filters(:,tt))));
        ...
        for nn=1:1:numbers_of_data
            ...
            random_color_img=zeros(size(seg_base_img));
            for ff=1:1:max(max(seg_base_img))
                random_color_img(find(seg_base_img==ff))=round(rand(1)*(end_colors-start_colors))+start_colors; % 각 영역에 랜덤하게 1-18 사이 숫자 부여
            end
            rotated_seg_img=imrotate(random_color_img,rand(1)*360-180);
            c_pos=[round(size(rotated_seg_img,1)/2),round(size(rotated_seg_img,2)/2)];
            cropped_img=rotated_seg_img(c_pos(1)-img_size(1)/2:c_pos(1)+img_size(1)/2-1,c_pos(2)-img_size(2)/2:c_pos(2)+img_size(2)/2-1);
